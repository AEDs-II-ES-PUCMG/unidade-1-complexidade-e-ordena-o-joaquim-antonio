...
public class App {
    ...
    /**
     * Vetor de pedidos ordenados pelo valor final (crescente) — usado para busca
     * binária em localizarPedidosPremium
     */
    ...
    static Comparator<Pedido> comparador;
    ...
    /**
     * Lê os dados de um arquivo-texto e retorna um vetor de pedidos. Arquivo-texto
     * no formato
     * N (quantidade de pedidos) <br/>
     * dataDoPedido;formaDePagamento;descrições dos produtos do pedido <br/>
     * Deve haver uma linha para cada um dos pedidos. Retorna um vetor vazio em caso
     * de problemas com o arquivo.
     * 
     * @param nomeArquivoDados Nome do arquivo de dados a ser aberto.
     * @return Um vetor com os pedidos carregados, ou vazio em caso de problemas de
     *         leitura.
     */
     ...
     * produto passado como parâmetro para esse método.
     * A busca não é sensível ao caso.
     * 
     * @param pesquisado Nome do produto a ser pesquisado no vetor de produtos
     *                   cadastrados.
    ...
    /**
     * Localiza e exibe todos os pedidos cujo valor final seja maior ou igual a um
     * valor de corte
     * informado pelo usuário.
     *
     * Estratégia otimizada: o vetor pedidosOrdenadosPorValor é mantido ordenado de
     * forma crescente
     * pelo valor final (ComparadorCriterioA). Uma busca binária identifica o
     * primeiro índice cujo
     * valorFinal() >= valorCorte em O(log n), evitando varredura completa do vetor.
     * Os pedidos
     * elegíveis — todos a partir desse índice — são impressos em O(k), onde k é o
     * número de
     * resultados encontrados, sem recalcular valores abaixo do corte.
     */
    static void localizarPedidosPremium(Pedido[] vetorOrdenado) {
        ...
        localizado = buscaBinaria(vetorOrdenado, 0, vetorOrdenado.length-1, valorCorte);
        System.out.println(localizado.toString());

    }

    static Pedido buscaBinaria(Pedido[] vet, int min, int max, double valorCorte) {

        int media = (min + max) / 2;

        if (vet[media].valorFinal() == valorCorte) {
            return vet[media];
        } else if (vet[media].valorFinal() < 0) {
            return buscaBinaria(vet, media, max, valorCorte);
        } else
            return buscaBinaria(vet, min, media, valorCorte);
    }
    ...
    /**
     * Ordena o vetor de pedidos cadastrados empregando um método de ordenação
     * selecionado pelo
     * usuário, dentre os seguintes: bolha, seleção, inserção, mergesort e heapsort.
     * O usuário também escolhe um critério de ordenação: A (Valor Final), B (Volume
     * de Itens)
     * ou C (Índice de Economia, decrescente).
     * O método interage com o usuário por meio de menus e aplica
     * a ordenação escolhida. Ao final, exibe o tempo total gasto no processo de
     * ordenação, em ms.
     */
    ...
    public static void main(String[] args) {
        ...
        // Cria cópia independente ordenada por valorFinal() (crescente) para busca
        // binária em localizarPedidosPremium
        ...
        do {
            ...
            switch (opcao) {
                ...
                case 4 -> localizarPedidosPremium(pedidosOrdenadosPorValor);
